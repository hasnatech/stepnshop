...
class Cart
{
	...
	public function getProducts()
	{
		...
		//print_r($this->db->escape($this->session->getId()));
		foreach ($cart_query->rows as $cart) {
			$stock = true;
	...
	{
		//$cart_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE (api_id > '0' OR customer_id = '0') AND date_added < DATE_SUB(NOW(), INTERVAL 1 HOUR) AND product_id='" . $product_id . "'");
		$cart_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'");
		print_r($this->db->escape($this->session->getId()));
		//return "SELECT * FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'";
		if ($cart_query->rows)
			return $cart_query->row['quantity'];
		else
			return 0;
	}
